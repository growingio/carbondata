...
import org.apache.carbondata.processing.datatypes.{
  ArrayDataType,
  GenericDataType,
  StructDataType
}
import org.apache.carbondata.processing.loading.CarbonDataLoadConfiguration
import org.apache.carbondata.processing.loading.complexobjects.{
  ArrayObject,
  StructObject
}
...
object CommonUtil {
  ...
  ): Option[util.List[ColumnProperty]] = {
    ...
    tableProperties.foreach {
      case (key, value) =>
        if (key.startsWith(columnPropertiesStartKey)) {
          fieldProps.add(
            ColumnProperty(
              key.substring(columnPropertiesStartKey.length(), key.length()),
              value
            )
          )
        }
  ...
  def validateTblProperties(tableProperties: Map[String, String],
        ...
        case (key, value) =>
          if (!validateFields(key, fields)) {
            ...
            throw new MalformedCarbonCommandException(
              s"Invalid table properties $key"
            )
          }
      }
    }
  ...
  def validateTypeConvertForSpark2(partitionerField: PartitionerField,
  ...
  def validateTypeConvert(partitionerField: PartitionerField,
                          value: String): Boolean = {
    val result = partitionerField.dataType.get.toLowerCase() match {
      case "int" =>
        scala.util.Try(value.toInt).isSuccess
    ...
    if (!result) {
      throw new MalformedCarbonCommandException(
        s"Invalid Partition Values for partition " +
  ...
  private def validateDateAndTimestamp(key: String,
                                       value: String,
                                       defaultValue: String) = {
  ...
  /**
    ...
    */
  def validateTableLevelCompactionProperties(
    tableProperties: Map[String, String]
  ): Unit = {
    ...
    validateCompactionPreserveSegmentsOrAllowedDays(
      tableProperties,
  ...
  /**
    ...
    * @param tableProperties
    */
  def validateMajorCompactionSize(
    tableProperties: Map[String, String]
  ): Unit = {
    ...
    if (tableProperties.get(tblPropName).isDefined) {
      ...
      try {
        ...
        case e: NumberFormatException =>
          throw new MalformedCarbonCommandException(
            s"Invalid $tblPropName value found: " +
  ...
  private def validateBoolean(tableProperties: Map[String, String],
                              property: String): Unit = {
    if (tableProperties.get(property).isDefined) {
      val trimStr = tableProperties(property).trim
      if (!trimStr.equalsIgnoreCase("true") && !trimStr.equalsIgnoreCase(
            "false"
          )) {
        throw new MalformedCarbonCommandException(
          s"Invalid $property value found: " +
  ...
  def validateAutoLoadMerge(tableProperties: Map[String, String]): Unit = {
    validateBoolean(
      tableProperties,
      CarbonCommonConstants.TABLE_AUTO_LOAD_MERGE
    )
  }
  ...
  /**
    ...
    * @param tableProperties
    */
  def validateCompactionLevelThreshold(
    tableProperties: Map[String, String]
  ): Unit = {
    ...
    if (tableProperties.get(tblPropName).isDefined) {
      ...
      try {
        ...
        val thresholds = regularedStr
          .split(",")
          .map(levelThresholdStr => levelThresholdStr.toInt)
  ...
  /**
    ...
    * @param tblPropName
    */
  def validateCompactionPreserveSegmentsOrAllowedDays(
    tableProperties: Map[String, String],
    tblPropName: String
  ): Unit = {
    if (tblPropName.equals(
          CarbonCommonConstants.TABLE_COMPACTION_PRESERVE_SEGMENTS
        ) ||
  ...
  /**
    ...
    */
  def validateSize(tableProperties: Map[String, String],
    ...
    if (tableProperties.get(propertyName).isDefined) {
      ...
      try {
        ...
        case e: NumberFormatException =>
          throw new MalformedCarbonCommandException(
            s"Invalid $propertyName value found: " +
  ...
  /**
    ...
    * @param propertyName property name
    */
  def validatePageSizeInmb(tableProperties: Map[String, String],
                           propertyName: String): Unit = {
    var size: Integer = 0
    if (tableProperties.get(propertyName).isDefined) {
      val pageSize: String =
  ...
  def parsePropertyValueStringInMB(propertyValueString: String): String = {
    ...
    if (propertyValueString.trim.toLowerCase.endsWith("mb")) {
      parsedPropertyValueString = propertyValueString.trim.toLowerCase
        .substring(0, propertyValueString.trim.toLowerCase.lastIndexOf("mb"))
        .trim
    }
    if (propertyValueString.trim.toLowerCase.endsWith("m")) {
      parsedPropertyValueString = propertyValueString.trim.toLowerCase
        .substring(0, propertyValueString.trim.toLowerCase.lastIndexOf("m"))
        .trim
    }
  ...
  def configureCSVInputFormat(configuration: Configuration,
    ...
    CSVInputFormat.setCommentCharacter(
      configuration,
      carbonLoadModel.getCommentChar
    )
    CSVInputFormat.setCSVDelimiter(
      configuration,
      carbonLoadModel.getCsvDelimiter
    )
    CSVInputFormat.setSkipEmptyLine(
      configuration,
      carbonLoadModel.getSkipEmptyLine
    )
    CSVInputFormat.setEscapeCharacter(
      configuration,
  ...
  def configSplitMaxSize(context: SparkContext,
                         filePaths: String,
    ...
    val spaceConsumed =
      FileUtils.getSpaceOccupied(filePaths, hadoopConfiguration)
    val blockSize =
      hadoopConfiguration.getLongBytes(
        "dfs.blocksize",
        CarbonCommonConstants.CARBON_256MB
      )
    ...
    if (spaceConsumed < defaultParallelism * blockSize) {
      ...
      LOGGER.info(
        s"totalInputSpaceConsumed: $spaceConsumed , " +
          s"defaultParallelism: $defaultParallelism"
      )
      LOGGER.info(
        s"mapreduce.input.fileinputformat.split.maxsize: ${newSplitSize.toString}"
      )
    }
  }
  ...
  def cleanInProgressSegments(databaseLocation: String,
                              dbName: String): Unit = {
    val loaderDriver = CarbonProperties
      .getInstance()
      .getProperty(
        CarbonCommonConstants.DATA_MANAGEMENT_DRIVER,
        CarbonCommonConstants.DATA_MANAGEMENT_DRIVER_DEFAULT
      )
      .toBoolean
    if (!loaderDriver) {
      return
        ...
        if (file.isDirectory) {
          ...
          tableFolders.foreach { tableFolder =>
            if (tableFolder.isDirectory) {
              ...
              val tableUniqueName =
                CarbonTable.buildUniqueName(dbName, tableFolder.getName)
              val tableStatusFile =
  ...
  def setTempStoreLocation(index: Int,
  ...
  def getTempStoreLocations(index: String): Array[String] = {
    ...
    val isCarbonUseYarnLocalDir = CarbonProperties
      .getInstance()
      .getProperty(
    ...
    val tmpLocationSuffix = s"${File.separator}carbon${UUID
      .randomUUID()
      .toString
      .replace("-", "")}${CarbonCommonConstants.UNDERSCORE}$index"
    if (isCarbonUseYarnLocalDir) {
      ...
      if (null != yarnStoreLocations && yarnStoreLocations.nonEmpty) {
        ...
        LOGGER.warn(
          "It seems that the we didn't configure local dirs for yarn," +
  ...
  /**
    ...
    * @param tableProperties
    */
  def validateCacheLevel(cacheLevel: String,
    ...
    if (cacheLevel.trim.isEmpty) {
      ...
      if (!supportedCacheLevel.contains(trimmedCacheLevel)) {
        val errorMessage =
          s"Invalid value: Allowed values for ${CarbonCommonConstants.CACHE_LEVEL} are BLOCK AND BLOCKLET"
        throw new MalformedCarbonCommandException(errorMessage)
      }
  ...
  /**
    ...
    * separated values
    */
  def validateColumnMetaCacheFields(
    dbName: String,
    tableName: String,
    ...
    if (!cachedColumns.equals("")) {
      ...
      val columnsToBeCached =
        cachedColumns.split(",").map(x => x.trim.toLowerCase).toSeq
      // make the columns in create table order and then add it to table properties
      val createOrder =
        tableColumns.filter(col => columnsToBeCached.contains(col))
      tableProperties.put(
  ...
  private def validateColumnMetaCacheOption(tableIdentifier: TableIdentifier,
    ...
    if (columnsToBeCached.trim.isEmpty) {
      ...
      val columns: Array[String] =
        columnsToBeCached.split(',').map(x => x.toLowerCase.trim)
      // Check for duplicate column names
      columns
        .groupBy(col => col.toLowerCase)
        .foreach(
          f =>
            if (f._2.size > 1) {
              throw new MalformedCarbonCommandException(
                s"Duplicate column name found : ${f._1}"
              )
          }
        )
      columns.foreach(col => {
        ...
        if (!tableColumns.contains(col.toLowerCase)) {
          val errorMessage =
            s"Column $col does not exists in the table ${tableIdentifier.table}"
          throw new MalformedCarbonCommandException(errorMessage)
        }
      })
    }
  ...
  def isDataTypeSupportedForSortColumn(columnDataType: String): Boolean = {
    val dataTypes =
      Array("array", "struct", "map", "double", "float", "decimal", "binary")
  ...
  def validateSortColumns(sortKey: Array[String],
    ...
    sortKey.foreach { column =>
      if (!fields.exists(x => x._1.equalsIgnoreCase(column))) {
        ...
        val dataType = fields.find(x => x._1.equalsIgnoreCase(column)).get._2
  ...
  def validateSortColumns(carbonTable: CarbonTable,
    ...
    if (!sortKeyString.isEmpty) {
      ...
      validateSortColumns(sortKey, fields.map { field =>
        (field.getColName, field.getDataType.getName)
      }, varcharCols)
    }
  }
    ...
    if (size.divideToIntegralValue(ONE_EB_BI).compareTo(BigDecimal.ZERO) > 0) {
      displaySize = size
        ...
        .doubleValue() + " EB"
                 ...
                 .divideToIntegralValue(ONE_PB_BI)
      ...
      displaySize = size
        ...
        .doubleValue() + " PB"
                 ...
                 .divideToIntegralValue(ONE_TB_BI)
                 .compareTo(BigDecimal.ZERO) > 0) {
      displaySize = size
        .divide(ONE_TB_BI)
        .setScale(2, RoundingMode.HALF_DOWN)
        .doubleValue() + " TB"
                 ...
                 .divideToIntegralValue(ONE_GB_BI)
                 .compareTo(BigDecimal.ZERO) > 0) {
      displaySize = size
        .divide(ONE_GB_BI)
        .setScale(2, RoundingMode.HALF_DOWN)
        .doubleValue() + " GB"
                 ...
                 .divideToIntegralValue(ONE_MB_BI)
                 .compareTo(BigDecimal.ZERO) > 0) {
      displaySize = size
        .divide(ONE_MB_BI)
        .setScale(2, RoundingMode.HALF_DOWN)
        .doubleValue() + " MB"
                 ...
                 .divideToIntegralValue(ONE_KB_BI)
                 .compareTo(BigDecimal.ZERO) > 0) {
      displaySize = size
        .divide(ONE_KB_BI)
        .setScale(2, RoundingMode.HALF_DOWN)
  ...
  def getObjectArrayFromInternalRowAndConvertComplexType(
    row: InternalRow,
    fieldTypes: Seq[DataType],
    outputArrayLength: Int
  ): Array[AnyRef] = {
      ...
      if (!row.isNullAt(i)) {
        fieldTypes(i) match {
          ...
          case arrayType: ArrayType =>
            data(i) =
              convertSparkComplexTypeToCarbonObject(row.getArray(i), arrayType)
          case structType: StructType =>
            data(i) = convertSparkComplexTypeToCarbonObject(
              row.getStruct(i, structType.fields.length),
              structType
            )
          case mapType: MapType =>
            data(i) =
              convertSparkComplexTypeToCarbonObject(row.getMap(i), mapType)
          case other =>
    ...
    while (i < fieldTypesLen) {
      fields(i).dataType match {
        ...
        case dataType @ (_: ArrayType | _: MapType) =>
          val result = convertSparkComplexTypeToCarbonObject(
            row.get(i, dataType),
            dataType
          )
          // convert carbon complex object to byte array
          val byteArray: ByteArrayOutputStream = new ByteArrayOutputStream()
          val dataOutputStream: DataOutputStream = new DataOutputStream(
            byteArray
          )
          dataFieldsWithComplexDataType(fields(i).name)
            .asInstanceOf[ArrayDataType]
            .writeByteArray(
              result.asInstanceOf[ArrayObject],
              dataOutputStream,
              badRecordLogHolder,
              true
            )
        ...
        case structType: StructType =>
          ...
          dataFieldsWithComplexDataType(fields(i).name)
            .asInstanceOf[StructDataType]
            .writeByteArray(
              result.asInstanceOf[StructObject],
              dataOutputStream,
              badRecordLogHolder,
              true
            )
  ...
  private def convertSparkComplexTypeToCarbonObject(
    data: AnyRef,
    ...
    objectDataType match {
      case _: ArrayType =>
        ...
        var i = 0
        while (i < size) {
          arrayChildObjects(i) = convertSparkComplexTypeToCarbonObject(
            arrayData.get(i, childDataType),
            childDataType
          )
          i = i + 1
        }
      ...
      case _: MapType =>
        ...
        while (i < size) {
          ...
          structChildObjects(0) = convertSparkComplexTypeToCarbonObject(
            keys.get(i, keyDataType),
            keyDataType
          )
          structChildObjects(1) = convertSparkComplexTypeToCarbonObject(
            values.get(i, valueDataType),
            valueDataType
          )
      ...
      case _: StructType =>
        ...
        while (i < size) {
          structChildObjects(i) = convertSparkComplexTypeToCarbonObject(
            structData.get(i, childrenSchema(i).dataType),
            childrenSchema(i).dataType
          )
          i = i + 1
        }
      ...
      case _: DateType =>
        if (data == null) {
          ...
          (data.asInstanceOf[Int] + DateDirectDictionaryGenerator.cutOffDate)
            .asInstanceOf[AnyRef]
        }
